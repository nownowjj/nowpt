...
const CalendarRecordNewOrFixPage = () => {
    ...
    const [importYn , setImportYn]  = useState(initialImportYn);
    ...
    let param = {};
        ...
        param.calendarSn = state.sn && state.sn ;   // sn의 존재 유무로 new , fix 구분

    const newRecordEvent=()=>{
        param.title   = titleValue;
        param.content = contentValue;
        param.importYn = importYn;
        ...
        if (validationResult !== true) {
            alertFunction(null , validationResult);
            return;
        }
